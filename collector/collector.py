    ...
    def collect_leader_init_details(self, leader_db_id, leader_fullName):
        ...
        if len(leader_info) == 1:
            for details in leader_info:
                profile_image = details.profile_image_url[0:details.profile_image_url.find('_normal')] + "_400x400" + details.profile_image_url[details.profile_image_url.find('.jpeg'):len(details.profile_image_url)]
                self.db.update_leader_details_regular("opinion_leaders", leader_db_id, details.id, details.screen_name, details.location, details.description, details.followers_count, details.friends_count, details.created_at, details.statuses_count, False, 10, profile_image)
        ...
        for post in result:
            ...
            if not self.check_if_post_exists_in_db(leader_db_id, post_id):
                ...
                if post_text[0] + post_text[1] == "RT":
                    ...
                    self.db.insert_post("posts", leader_twitter_id, post_date, post_id, post_text, post.retweet_count, post.retweeted_status.favorite_count)
                elif "quoted_status_id" in post:
                    quoted_status_id = post.quoted_status_id
                    print(quoted_status_id)
                else:
                    """ Handles regular tweet """
                    self.db.insert_post("posts", leader_twitter_id, post_date, post_id, post_text, post.retweet_count, post.favorite_count)
