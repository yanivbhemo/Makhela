...
#             try:
#                 if value['posts']:
#                     self.leaders[key]['topics'] = self.analyze_lda(value['posts'])
#                 else:
...
#             try:
#                 topics[key] = self.analyze_lda(value)
#             except:
...
#         try:
#             posts_topics = self.analyze_lda(posts_data)
#         except:
...
#             except:
#                 print("exception while finding important wordst: ", i, words_df.loc[i, 'words'])
#                 continue
#
#         return
#
...
#                     except:
#                         print("failed adding community to ", community[0])
#                         pass
#                 else:
#                     for c in community:
#                         try:
...
class Analyzer:
    ...
    def analyze_community(self):
        #         log = logger.logger_handler()
        print("Beginning to analyze community")

        self.add_key_words()
        self.important_words()

        print("Analyzing LDA Leaders")
        for key, value in self.leaders.items():
            try:
                if value['posts']:
                    self.leaders[key]['topics'] = self.analyze_lda(value['posts'])
                ...
                print("Failed LDA leader", key)

        self.build_graph()
        self.centrality()
        self.community()

        print("Analyzing LDA - Communities")
        community_posts = {}
        for key, value in self.leaders.items():
            try:
                community_posts[value['community']] += value['posts']
        ...
        for key, value in community_posts.items():
            try:
                ...
                print("exception LDA", key)

        print("Analyzing LDA - Network")
        posts_data = []
        for key, value in self.posts.items():
            posts_data.append(value['full_text'])
        try:
    ...
    def add_key_words(self):
        print("adding key word indicator to post")
        for key, value in self.posts.items():
            try:
                word_found = 0
                for key_word in self.key_words:
                ...
                print("exception adding key word indicator to post: ", key)
                continue
    ...
    def important_words(self):
        ...
        for key, value in self.posts.items():
            try:
                ...
                print("exception finding text in post: ", key)
                continue
        words_df = pd.DataFrame(words_from_posts)
        vectorizer = TfidfVectorizer()
        vectors = vectorizer.fit_transform(words_df[1])
    ...
    def centrality(self):
        ...
        try:
            ...
            print("exception finding degree_centrality")
        try:
            betweenness_centrality = nx.betweenness_centrality(self.DG)
        except:
            print("exception finding betweenness_centrality")
        try:
            closeness_centrality = nx.closeness_centrality(self.DG)
        except:
            print("exception finding closeness_centrality")

        for key, value in self.leaders.items():
            try:
                self.leaders[key]["deg_centrality"] = deg_centrality[key]
            except:
                print("exception adding deg_centrality: ", key)
                continue
        try:
            self.leaders[key]["betweenness_centrality"] = betweenness_centrality[key]
        except:
            print("exception adding betweenness_centrality: ", key)
            pass
        try:
            self.leaders[key]["closeness_centrality"] = closeness_centrality[key]
        except:
            print("exception adding closeness_centrality: ", key)
            pass
                ...
                if len(community) == 1:
                    ...
                    for c in community:
                        try:
                            ...
                            print("failed adding community to ", c)
                            continue
                    counter += 1
        except:
            print("failed analyzing community")
